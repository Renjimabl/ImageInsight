...
# Suppress warnings
warnings.filterwarnings("ignore", category=UserWarning, message=".*oneDNN custom operations are on.*")
warnings.filterwarnings("ignore", category=FutureWarning, message=".*resume_download is deprecated.*")
warnings.filterwarnings("ignore", category=UserWarning, message=".*torch.load.*")
warnings.filterwarnings("ignore", category=UserWarning, message=".*tf.reset_default_graph.*")
...
def get_caption_with_blip(image):
    inputs = processor(image, return_tensors="pt").to(vqa_model.device)
...
def get_answer_with_vqa(image, question):
    # Pass both image and question to the VQA model
    inputs = processor(image, question=question, return_tensors="pt").to(vqa_model.device)
    # Generate answer
    outputs = vqa_model.generate(**inputs, max_new_tokens=20)
    return processor.decode(outputs[0], skip_special_tokens=True)
            ...
            if question:
                ...
                try:
                    with st.spinner("Getting answer..."):
                        answer = get_answer_with_vqa(image, question)
